...
/**
 * Netty channel handler that proxies HTTP requests to origin servers.
 * This is a refactored version of ProxyEndpoint that works as a low-level Netty channel handler
 * instead of a Zuul Filter.
 * 
 * Not thread safe! New instance of this class is created per HTTP/1.1 request proxied to the origin but NOT for each
 * attempt/retry. All the retry attempts for a given HTTP/1.1 request proxied share the same ProxyChannelHandler instance
 */
public class ProxyChannelHandler extends ChannelDuplexHandler implements GenericFutureListener<Future<PooledConnection>> {
    ...
    protected HttpRequestMessage zuulRequest;
    protected SessionContext context;

    @Nullable
    protected NettyOrigin origin;

    protected RequestAttempts requestAttempts;
    protected CurrentPassport passport;
    protected NettyRequestAttemptFactory requestAttemptFactory;
    protected OriginTimeoutManager originTimeoutManager;
    ...
    private static final Logger logger = LoggerFactory.getLogger(ProxyChannelHandler.class);
    ...
    public static final String CHANNEL_HANDLER_NAME = "_proxy_channel_handler";

    public ProxyChannelHandler() {
        this(new NettyRequestAttemptFactory());
    }

    public ProxyChannelHandler(NettyRequestAttemptFactory requestAttemptFactory) {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        try (TaskCloseable ignore = PerfMark.traceTask("ProxyChannelHandler.channelRead")) {
            if (msg instanceof HttpRequestMessage) {
                handleHttpRequest(ctx, (HttpRequestMessage) msg);
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, HttpRequestMessage incomingRequest) throws Exception {
        // Initialize state for this request
        zuulRequest = transformRequest(incomingRequest);
        ...
        if (origin == null) {
            handleNoOriginSelected(ctx);
            return;
        }

        try {
            origin.onRequestExecutionStart(zuulRequest);
            proxyRequestToOrigin(ctx);
        } catch (Exception ex) {
            handleError(ctx, ex);
        }
    }
    ...
    private void handleHttpContent(ChannelHandlerContext ctx, HttpContent chunk) throws Exception {
        if (originConn != null) {
            ...
            ByteBufUtil.touch(chunk, "ProxyChannelHandler writing chunk to origin, request: ", zuulRequest);
            originConn.getChannel().writeAndFlush(chunk);
        } else {
            // Not connected to origin yet, pass through to let caller buffer the request body
    ...
    public void invokeNext(ChannelHandlerContext ctx, final HttpResponseMessage zuulResponse) {
        try {
            filterResponse(ctx, zuulResponse);
        } catch (Exception ex) {
            unlinkFromOrigin();
    ...
    private void filterResponse(ChannelHandlerContext ctx, final HttpResponseMessage zuulResponse) {
        // Send response down the pipeline
        ctx.fireChannelRead(zuulResponse);
    }

    public void invokeNext(ChannelHandlerContext ctx, final HttpContent chunk) {
        try {
            ByteBufUtil.touch(chunk, "ProxyChannelHandler received chunk from origin, request: ", zuulRequest);
            filterResponseChunk(ctx, chunk);
        } catch (Exception ex) {
            ByteBufUtil.touch(chunk, "ProxyChannelHandler exception processing chunk from origin, request: ", zuulRequest);
            unlinkFromOrigin();
    ...
    private void filterResponseChunk(ChannelHandlerContext ctx, final HttpContent chunk) {
        if (context.isCancelled() || !ctx.channel().isActive()) {
    ...
    private void proxyRequestToOrigin(ChannelHandlerContext ctx) {
        ...
        try {
            ...
            promise = origin.connectToOrigin(
                    zuulRequest, ctx.channel().eventLoop(), attemptNum, passport, chosenServer, chosenHostAddr);
    ...
    public void operationComplete(final Future<PooledConnection> connectResult) {
        try {
            ...
            logger.error(
                    ...
                    ChannelUtils.channelInfoForLogging(((ChannelHandlerContext) context.get(CommonContextKeys.NETTY_SERVER_CHANNEL_HANDLER_CONTEXT)).channel()),
                    ex);

            unlinkFromOrigin();

            // Fire exception here to ensure that server channel gets closed, so clients don't hang.
            ChannelHandlerContext channelCtx = (ChannelHandlerContext) context.get(CommonContextKeys.NETTY_SERVER_CHANNEL_HANDLER_CONTEXT);
            if (channelCtx != null) {
    ...
    private void onOriginConnectFailed(Throwable cause) {
        ...
        if (!context.isCancelled()) {
            ChannelHandlerContext channelCtx = (ChannelHandlerContext) context.get(CommonContextKeys.NETTY_SERVER_CHANNEL_HANDLER_CONTEXT);
            errorFromOrigin(channelCtx, cause);
        }
    }
    ...
    protected OriginResponseReceiver getOriginResponseReceiver() {
        return new ProxyOriginResponseReceiver(this);
    }
    ...
    public void errorFromOrigin(ChannelHandlerContext ctx, final Throwable ex) {
        try {
            ...
            processErrorFromOrigin(ctx, ex, originCh);
        } catch (Exception e) {
            ctx.fireExceptionCaught(ex);
        }
    }

    private void processErrorFromOrigin(ChannelHandlerContext ctx, final Throwable ex, final Channel origCh) {
        try {
    ...
    private void handleNoOriginSelected(ChannelHandlerContext ctx) {
    ...
    public void responseFromOrigin(final HttpResponse originResponse) {
        try (TaskCloseable ignore = PerfMark.traceTask("ProxyChannelHandler.responseFromOrigin")) {
            PerfMark.attachTag("uuid", zuulRequest, r -> r.getContext().getUUID());
            PerfMark.attachTag("path", zuulRequest, HttpRequestInfo::getPath);
            ByteBufUtil.touch(originResponse, "ProxyChannelHandler handling response from origin, request: ", zuulRequest);
            ChannelHandlerContext channelCtx = (ChannelHandlerContext) context.get(CommonContextKeys.NETTY_SERVER_CHANNEL_HANDLER_CONTEXT);
            processResponseFromOrigin(channelCtx, originResponse);
        } catch (Exception ex) {
            unlinkFromOrigin();
    ...
    private void processResponseFromOrigin(ChannelHandlerContext ctx, final HttpResponse originResponse) {
        if (originResponse.status().code() >= 500) {
            handleOriginNonSuccessResponse(ctx, originResponse, chosenServer.get());
        } else {
            handleOriginSuccessResponse(ctx, originResponse, chosenServer.get());
        }
    }

    protected void handleOriginSuccessResponse(ChannelHandlerContext ctx, final HttpResponse originResponse, DiscoveryResult chosenServer) {
        ...
        ByteBufUtil.touch(originResponse, "ProxyChannelHandler handling successful response, request: ", zuulRequest);
    ...
    private HttpResponseMessage buildZuulHttpResponse(
        ...
        if (httpResponse instanceof DefaultFullHttpResponse) {
            ByteBufUtil.touch(
                    httpResponse, "ProxyChannelHandler converting Netty response to Zuul response, request: ", zuulRequest);
    ...
    protected void handleOriginNonSuccessResponse(ChannelHandlerContext ctx, final HttpResponse originResponse, DiscoveryResult chosenServer) {
        final int respStatus = originResponse.status().code();
        ...
        if (retryable5xxResponse && isBelowRetryLimit()) {
            ...
            // detach from current origin.
            ByteBufUtil.touch(originResponse, "ProxyChannelHandler handling non-success retry, request: ", zuulRequest);
            unlinkFromOrigin();
            releasePartialResponse(originResponse);
            ...
            ByteBufUtil.touch(originResponse, "ProxyChannelHandler handling non-success response, request: ", zuulRequest);
    ...
    /**
     * Custom OriginResponseReceiver that delegates back to this ProxyChannelHandler
     */
    private static class ProxyOriginResponseReceiver extends OriginResponseReceiver {
        ...
        protected void channelReadInternal(ChannelHandlerContext ctx, Object msg) throws Exception {
            if (msg instanceof HttpResponse) {
                if (proxyHandler != null) {
                    proxyHandler.responseFromOrigin((HttpResponse) msg);
                ...
                if (proxyHandler != null) {
                    ChannelHandlerContext channelCtx = (ChannelHandlerContext) proxyHandler.context.get(CommonContextKeys.NETTY_SERVER_CHANNEL_HANDLER_CONTEXT);
                    proxyHandler.invokeNext(channelCtx, chunk);
                } else {
                    ReferenceCountUtil.safeRelease(chunk);
                }
                ctx.channel().read();
                ...
                if (proxyHandler != null) {
                    ChannelHandlerContext channelCtx = (ChannelHandlerContext) proxyHandler.context.get(CommonContextKeys.NETTY_SERVER_CHANNEL_HANDLER_CONTEXT);
                    proxyHandler.errorFromOrigin(channelCtx, error);
                }
    ...
    private void handleError(ChannelHandlerContext ctx, final Throwable cause) {
