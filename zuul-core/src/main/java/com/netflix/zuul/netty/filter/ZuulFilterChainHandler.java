...
public class ZuulFilterChainHandler extends ChannelInboundHandlerAdapter {
    ...
    protected void fireEndpointFinish(final boolean error, final ChannelHandlerContext ctx) {
        ...
        if (endpoint instanceof ProxyChannelHandlerAdapter) {
            final ProxyChannelHandlerAdapter proxyAdapter = (ProxyChannelHandlerAdapter) endpoint;
            proxyAdapter.finish(error);
        }
        zuulRequest = null;
    }
